...
const getNetworkInfo = () => {
    ...
    return {
        ...
        saveData: typeof conn.saveData === "boolean" ? (conn.saveData ? "On" : "Off") : "NA",
    };
};
...
function IP() {
    ...
    const ispRows = info
        ? [
            { label: "Status", value: typeof info.success === "boolean" ? (info.success ? "Success" : "Failed") : null },
            { label: "Type", value: info.type },
            { label: "Continent", value: info.continent_code ? `${info.continent}` : info.continent },
            { label: "Country", value: info.country_code ? `${info.country}` : info.country },

            { label: "Capital", value: info.capital },
            { label: "ISP", value: info.connection?.isp },
            { label: "Timezone", value: info.timezone?.abbr ? `${info.timezone?.id} (${info.timezone.abbr})` : info.timezone?.id },
            { label: "UTC", value: info.timezone?.utc },

        ].filter((row) => row.value !== null && row.value !== undefined && row.value !== "")
        : [];

    const networkRows = [
        { label: "Network Quality", value: network.effectiveType },
        { label: "Estimated Speed", value: network.downlink },
    ...
    if (error) {
        return (
            <>
                <p style={{ color: "rgba(255,255,255,0.75)", margin: "0 0 1rem" }}>
